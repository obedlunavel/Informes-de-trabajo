...
class DetallesExpediente:
    ...
    def extraer_nuevos_objetivos(self, nuevos_obj_frame):
        """Extrae datos del frame actual incluyendo todos los niveles de subframes."""
        nuevos_objetivos = []
        
        def obtener_entries(frame):
            """Función recursiva mejorada con depuración."""
            entries = []
            try:
                ...
                print(f"\n🔍 Analizando frame: {frame}")
                print(f"Widgets hijos: {frame.winfo_children()}")
                
                for widget in frame.winfo_children():
                    try:
                        # Debug: Mostrar tipo de widget
                        print(f"  Widget: {widget} | Tipo: {type(widget)}")
                        
                        if isinstance(widget, tk.Entry):
                            valor = widget.get().strip()
                            if valor:
                                print(f"    ✅ Entry válido: {valor}")
                                entries.append(valor)
                        elif isinstance(widget, tk.Frame):
                            print("    🧩 Subframe detectado, buscando recursivamente...")
                            entries.extend(obtener_entries(widget))
                            
                    except tk.TclError as e:
                        print(f"    🚨 Widget inválido: {str(e)}")
                        continue
                        
            except tk.TclError as e:
                print(f"🚨 Error en frame: {str(e)}")
                
            return entries

        try:
            if nuevos_obj_frame and nuevos_obj_frame.winfo_exists():
                print("\n🔥 Iniciando extracción desde frame actual")
                nuevos_objetivos = obtener_entries(nuevos_obj_frame)
                
                # Debug: Mostrar resultados preliminares
                print(f"\n📊 Resultados preliminares: {nuevos_objetivos}")
                
                # Forzar actualización GUI antes de retornar
                nuevos_obj_frame.update_idletasks()
            else:
                print("⚠️ Usando datos de respaldo de campos_modificados")
                nuevos_objetivos = self.campos_modificados.get("nuevos_objetivos", [])
                
        except tk.TclError as e:
            print(f"🔥 Error crítico: {str(e)}")
            nuevos_objetivos = self.campos_modificados.get("nuevos_objetivos", [])

        # Actualizar siempre los datos y forzar sincronización
        self.campos_modificados["nuevos_objetivos"] = nuevos_objetivos
        print(f"\n🔄 Datos finales actualizados: {nuevos_objetivos}")
