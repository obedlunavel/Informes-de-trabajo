...
class ExpedientesApp:
    ...
    def cargar_configuracion_desde_archivo(self):
        ...
        if not os.path.exists(archivo_config):
            messagebox.showinfo("Error", "No se encontró el archivo de configuración.")
            return None
        ...
        try:
            ...
            messagebox.showinfo("Error", f"Error al leer el archivo de configuración: {e}")
            return None
    ...
    def ver_detalles(self):
        ...
        campos_necesarios = [
            "fecha", "unidad", "expediente", "nombre", "edad", "fecha_nacimiento",
            "diagnostico", "area_intervencion", "periodo_intervencion",
            "terapias_recibidas", "faltas", "objetivos_iniciales", "avance_objetivos",
            "nuevos_objetivos", "observaciones", "tratamiento", "sugerencias_casa",
            "elaborado_por", "cedula"
        ]

        # Agregar campos faltantes con valores predeterminados
        for campo in campos_necesarios:
            if campo not in expediente:
                expediente[campo] = "No especificado"

        # Obtener datos de configuración y actualizar campos
        config = self.cargar_configuracion_desde_archivo()
        if config:
            expediente["elaborado_por"] = config.get("nombre_terapeuta", "No especificado")
            expediente["cedula"] = config.get("cedula_profesional", "No especificado")
        else:
            expediente["elaborado_por"] = "No especificado"
            expediente["cedula"] = "No especificado"
